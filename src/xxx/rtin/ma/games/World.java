...
public class World {
    ...
    private World() {
        mDebris = new Vector2f[100];
        for(int i=0; i<100; ++i) {
            mDebris[i] = new Vector2f(mRandom.nextFloat() * 1600, mRandom.nextFloat() * 1200);
        }
    }
	...
	private void resolveCollision(GameEntity e1, GameEntity e2) {
	    ...
	    if(r1 < r2) {
	        ...
	        /*
	        //reflect through perp.
	        toTarget.negateLocal();
	        float targetAngle = (float) toTarget.getTheta();
	        targetAngle += 90;
	        
	        float difference = StaticUtil.AngleBetween(targetAngle, (float)e1.getVel().getTheta());
	        targetAngle -= difference;
	        	        
	        float v = e1.getVel().length();
            float rads = (float) Math.toRadians(targetAngle);
            
	        e1.getVel().set((float)(v * FastTrig.cos(rads)), (float)(v * FastTrig.sin(rads)));
	        */
	    } else {
	        //e2 smallest
            ...
            float difference = StaticUtil.AngleBetween(targetAngle, (float)e2.getVel().getTheta());
            targetAngle += difference;
            
            float v = e2.getVel().length();
            float rads = (float) Math.toRadians(targetAngle);
            
            e2.getVel().set((float)(v * FastTrig.cos(rads)), (float)(v * FastTrig.sin(rads)));
            */
	    }
	}
...
/*
        //which multiple of 1600 we have so far
	    ...
	    for(Vector2f v : mDebris) {

	        
	        g.drawString("*", 1600*xf + v.x, 1200*yf + v.y);
	        g.drawString("*", 1600*(xf+1) + v.x , 1200*yf + v.y);
